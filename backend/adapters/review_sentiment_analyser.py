...
Reads reviews from a CSV (column 'review_text' required), runs:
 - Zero-shot classification to detect which aspects/categories a sentence mentions (default)
     OR fast keyword matching (use --aspect_method keywords)
 - Sentiment analysis on sentences (cached to avoid duplicate calls)
Aggregates:
 - Per-review sentiment
 - Per-category sentiment (aggregated across sentences/reviews)
 - Overall numeric rating (1-5)
Performance flags:
    --aspect_method zsc|keywords   Choose slow accurate zero-shot vs fast keyword heuristic (default: zsc)
    --zsc_batch_size N             Batch size for zero-shot pipeline (default 4)
    --sent_batch_size N            Batch size for sentiment pipeline (default 16)
    --cache_sentiment              Enable in-memory cache of sentence sentiment (recommended for many overlapping sentences)
    --skip_plots                   Skip generating PNG charts (saves time)
...
Usage:
    ...
    python review_sentiment_analyser.py --input reviews.csv --aspect_method keywords --cache_sentiment --skip_plots --sent_batch_size 64 --timings

    for fast-balanced performance:
    python review_sentiment_analyser.py --input reviews.csv --aspect_method keywords --cache_sentiment --skip_plots --sent_batch_size 32 --timings
...
def analyze_reviews(df,
    ...
    if collect_timings:
        timings['load_zero_shot'] = t1 - t0
        timings['load_sentiment'] = t2 - t1
    ...
    # sentiment cache (sentence -> (norm_label, confidence, numeric_score))
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Analyzing reviews"):
        ...
        if aspect_method == 'zsc':
            zsc_results = zsc(sentences, candidate_labels=candidate_aspects, multi_label=True, batch_size=zsc_batch_size)
            ...
            # keyword heuristic: if aspect term appears in sentence -> add with pseudo score 1.0
            for s in sentences:
                low = s.lower()
                for aspect in candidate_aspects:
                    if aspect.lower() in low:
                        per_category_sentences[aspect].append((s, 1.0))
        ...
        for cat, sents in per_category_sentences.items():
            ...
            # Determine which sentences need inference
            to_infer = [s for s in texts_for_sentiment if (not cache_sentiment) or (s not in sentiment_cache)]
            if to_infer:
                sent_out_new = sentiment(to_infer, batch_size=sent_batch_size)
                for s_text, o in zip(to_infer, sent_out_new):
                    raw_label = o['label']
                    conf = o.get('score', 1.0)
                    norm_label = normalize_sentiment_label(raw_label)
                    numeric_score = map_sentiment_label_to_score(norm_label, conf)
                    if cache_sentiment:
                        sentiment_cache[s_text] = (norm_label, conf, numeric_score)
            # Build output objects in original order
            sent_out = []
            for s_text in texts_for_sentiment:
                if cache_sentiment and s_text in sentiment_cache:
                    norm_label, conf, numeric_score = sentiment_cache[s_text]
                    sent_out.append({'label': norm_label, 'score': conf, '_numeric': numeric_score})
                else:
                    # Should not happen unless caching off
                    o = sentiment(s_text)[0]
                    norm_label = normalize_sentiment_label(o['label'])
                    conf = o.get('score', 1.0)
                    numeric_score = map_sentiment_label_to_score(norm_label, conf)
                    sent_out.append({'label': norm_label, 'score': conf, '_numeric': numeric_score})
            ...
            for o, original_text in zip(sent_out, texts_for_sentiment):
                norm_label = normalize_sentiment_label(o['label'])
                conf = o.get('score', 1.0)
                numeric_vals.append(o.get('_numeric', map_sentiment_label_to_score(norm_label, conf)))
        ...
        if cache_sentiment and text in sentiment_cache:
            overall_label = sentiment_cache[text][0]
            overall_score_raw = sentiment_cache[text][1]
            overall_numeric = sentiment_cache[text][2]
        else:
            ...
            if cache_sentiment:
                sentiment_cache[text] = (overall_label, overall_score_raw, overall_numeric)
    ...
    if collect_timings:
        timings['analysis_loop'] = loop_end - loop_start
    meta = {
...
def save_outputs(outdir: Path, results_df: pd.DataFrame, category_summary_df: pd.DataFrame, top_keywords: dict, meta: dict, generate_plots: bool = True):
    ...
    if generate_plots:
        ...
        # 3) Top positive/negative keywords
...
def main():
    ...
    parser.add_argument('--aspect_method', type=str, choices=['zsc','keywords'], default='zsc', help='Aspect extraction method: zero-shot (accurate) or keywords (fast)')
    parser.add_argument('--zsc_batch_size', type=int, default=4, help='Batch size for zero-shot pipeline calls')
    parser.add_argument('--sent_batch_size', type=int, default=16, help='Batch size for sentiment pipeline calls')
    parser.add_argument('--cache_sentiment', action='store_true', help='Cache sentence sentiment results to speed up processing')
    parser.add_argument('--skip_plots', action='store_true', help='Skip generating plots for speed')
    ...
    results_df, category_summary_df, top_keywords, meta = analyze_reviews(
        ...
        cache_sentiment=args.cache_sentiment,
    ...
    save_outputs(outdir, results_df, category_summary_df, top_keywords, meta, generate_plots=not args.skip_plots)
    ...
    if args.timings and meta.get('timings'):
        print("\n=== TIMINGS (seconds) ===")
        for k,v in meta['timings'].items():
            print(f"  {k}: {v:.3f}")
        print("(Use --aspect_method keywords + --cache_sentiment + --skip_plots for fastest run)")
